...
# Don't mock - use proper conda environment
...
print(f'Marker IDs: {list(can_mrk_ids.keys())}')
...
print(f'  exists: {osp.exists(marker_layout_fname)}')
...
for i, fv in enumerate(free_vars):
    ...
    try:
        ...
        import traceback
        t1 = time.time()
        print(f'  Error after {t1-t0:.3f}s: {e}')
        traceback.print_exc()
